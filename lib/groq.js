...
// Groq "compound" systems have built-in web search — they look up live nutrition data
// for branded/regional foods the base model may not know. compound-mini is ~3x faster.
// Hard-coded (no env needed); if a key lacks compound access the call 404s and callers
// fall back to TEXT_MODEL.
...
export async function chatJSON(groq, { model, messages, temperature = 0.4, maxTokens = 2048, retries = 1, reasoning = true }) {
  let lastErr;
  ...
  // Pass reasoning:false for models that don't accept the param at all (e.g. compound).
  let useReasoningEffort = reasoning;
